...
### Slide 2 — From Halawi to live markets  (30 sec) · **transition 1/2**
...
> "So we asked: *where can we cleanly test 'calibrated model beats crowd' with fast resolution, real money, and a documented bias?*"

### Slide 3 — Why NBA, not crypto  (30 sec) · **transition 2/2**
> "We considered two natural laboratories: crypto 5-minute lead-lag, and NBA in-play markets."
> *(walk the row labels)*
> "NBA gives clean labels — every game state has an outcome, so it's a **calibration problem** instead of a stat-arb on noisy proxies."
> "The literature for NBA is *positive* — Moskowitz 2021 in the Journal of Finance documents the exact bias. Crypto's nearest analog is a *negative* result."
> "And practically: Kalshi NBA event contracts are CFTC-regulated and legal in California."
> "Trade *with* the literature, on the venue we can use. **End of transition. The rest is NBA.**"
...
> "Here's the whole system in one diagram. **Top row is the model side**: play-by-play, 2,460 games from nba_api, turned into per-minute game-state snapshots, fed to V2 — XGBoost plus isotonic — outputs `p̂_t`, our calibrated probability the home team wins the 1st half."
...
### Slide 6 — V2 model  (45 sec)
> "V2 itself is intentionally small. XGBoost on four features: `minute_idx`, `score_diff_home`, `recent_run_diff`, `period`. Isotonic on a held-out fold so a 0.70 prediction actually corresponds to a 70% empirical frequency."
...
### Slide 8 — Mispricing thesis (V5)  (75 sec) · **headline #2**
> "Now the behavioral test. For each pre-registered event, we measure the **structural shift for the scorer over the next 60 seconds** — how much does our calibrated model move toward the scoring team after they make a basket while trailing?"
> *(point at the bars)*
...
> "We also have a five-game Kalshi archive backtest. The *same model family*, against Kalshi's 1st-half-winner market, returns plus-100-percent."
...
> "V3 — our planned variant — is the literal Halawi analog: `p̂_aggregate = w·p_model + (1−w)·p_market_devig`, weight chosen by validation Brier. The aggregate's Brier is bounded by the minimum of the components."
...
> "Limitations, briefly: sample size — we'd want roughly 1,000 liquid-market games for a powered backtest. Horizon — V2 is 1st-half-only. Reactivity — sportsbooks limit winners; Kalshi as peer-to-peer sidesteps that. And V1, V3, V4, V6 are architecturally ready but need multi-venue history."

### Slide 14 — Verdict  (45 sec)
> "Two questions, answered directly."
> *(point at left panel)*
> "**Which mispricing method works best?** V5 — event-conditioned overreaction. It's the only variant with a statistically significant finding on the held-out test season: H1 plus 0.0075 at p less than 0.0001, H4 plus 0.0138 at p less than 0.0001. The bias is statistically real."
> *(point at right panel)*
> "**Can it make money?** Not today, but it's not 'no.' Today we can't claim profit — one liquid game is noise, the Kalshi plus-95% is a stale-mid artifact. But the bias is real, and the path forward is concrete: power the backtest with the paid historical season, then deploy a V5-targeted strategy on Kalshi, which is legal and has no account-limit risk."
...
```
   ...
   ├─────────────────────────────────────────────────────────────┤
   │ 3. MODEL V2     XGBoost + isotonic → P(home wins 1H)         │
   │                 ✅ OOS Brier 0.149 · ECE 0.008                │
   ├─────────────────────────────────────────────────────────────┤
   │ 4. EVAL HARNESS metrics · game-level splits · backtest       │
   │                 5/5 honesty gates pass on synthetic data     │
   ├─────────────────────────────────────────────────────────────┤
   │ 5. MISPRICING   V5 event overreaction · pre-registered       │
   │      (V5)       ✅ H1 +0.0075 p<0.0001 · H4 +0.0138 p<0.0001  │
   ├─────────────────────────────────────────────────────────────┤
   ...
   ├─────────────────────────────────────────────────────────────┤
   │ 9. DECK + DOCS   14-slide pptx · how_it_works · variant strat │
   └─────────────────────────────────────────────────────────────┘
```

**Built but underpowered (need more data):** V1 cross-venue consensus, V3 Halawi aggregate, V4 mean-reversion, V6 arbitrage descriptor, V5 *market-side* H1/H4 (vs structural-side which passed).
...
### Behavioral test (V5) ✅ both pre-registered tests pass
...
1. **Power the backtest.** $59 for the-odds-api historical 100K plan → pull the full 2024-25 in-play season → run all 6 variants through the harness → see which (if any) clear vig with a CI excluding zero. **This is the single decision that turns "maybe" into "yes/no with numbers."**
2. **Deploy on Kalshi where liquid.** When *real volume* is present (Finals games, marquee matchups), Kalshi is a peer-to-peer market with no account-limit risk, legal in California. Use V2/V5 + the full-game model, paper-trade first, then small stakes only after a powered backtest passes.
3. **Focus on the V5 event window specifically.** H1/H4 events are where the bias is documented and pre-registered. A V5-targeted strategy — only bet within ~60s of trailing-team makes — narrows the exposure to where the literature predicts edge actually lives.
